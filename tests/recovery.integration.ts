...
async function main() {
  ...
  try {
    ...
    const plan = planSchema.parse({ id: "project", name: "Project", sources: [source], destinationIds: ["a", "b"], enabled: true, includes: [], excludes: [], schedule: { kind: "manual" }, retention: { daily: 1, weekly: 0, monthly: 0 }, replicateFrom: "a", recoveryDrillDays: 0 });
    ...
    const pruneId = await service.request({ type: "retention", planId: plan.id, destinationId: "a", preview: false }) as string;
    const pendingCopy = await service.request({ type: "copy-snapshot", sourceDestinationId: "a", destinationId: "b", snapshotId: first.id }) as string;
    await assert.rejects(service.request({ type: "pin", destinationId: "a", snapshotId: first.id, pinned: true }), /pending copies/);
    await service.request({ type: "settings", settings: { ...service.state().settings, paused: false } });
    await wait([pendingCopy]);
    assert.equal(service.store.getJob(pruneId).status, "failed");
    assert.match(service.store.getJob(pruneId).error ?? "", /still need to be copied/);
    await wait([await service.request({ type: "retention", planId: plan.id, destinationId: "a", preview: false }) as string]);
    assert((await snapshots("a")).some(s => s.id === first.id));
    results.push("Pending copies prevent source pruning and snapshot-ID changes; future checkpoints survive explicit retention");
    ...
    await service.request({ type: "save-plan", plan: { ...plan, recoveryDrillDays: 0, backupOnConnect: true } });
    service.store.put("connected", "a", false);
    service.store.put("connected", "b", true);
    service.store.put("protection", plan.id + ":a", { planId: plan.id, destinationId: "a", lastSuccess: "2020-01-01T00:00:00Z" });
    await service.request({ type: "settings", settings: { ...service.state().settings, paused: false } });
    await service.checkConnections();
    const connectionJobs = service.state().jobs.filter(j => j.trigger === "drive connected");
    assert.equal(connectionJobs.length, 1);
    await wait(connectionJobs.map(j => j.id));
    assert(service.state().finishedDrives?.includes("a"));
    const destinationA = service.state().destinations.find(d => d.id === "a")!;
    const destinationB = service.state().destinations.find(d => d.id === "b")!;
    service.store.put("destination", "a", { ...destinationA, location: destinationB.location });
    service.store.put("connected", "a", false);
    await service.checkConnections();
    assert.equal(service.state().jobs.filter(j => j.trigger === "drive connected").length, 1);
    service.store.put("destination", "a", destinationA);
    results.push("Emulated drive arrival queues overdue backup once and rejects a different repository at the same location");
    await snapshots("b"); // Rebuild the catalog intentionally deleted by the replacement-PC test.
    await service.request({ type: "save-plan", plan: { ...plan, recoveryDrillDays: 7 } });
    await service.tick();
    const scheduledDrills = service.state().jobs.filter(j => j.trigger === "recovery drill");
    assert.equal(scheduledDrills.length, 1);
    assert.equal(scheduledDrills[0].destinationId, "b");
    await wait(scheduledDrills.map(j => j.id));
    await service.tick();
    assert.equal(service.state().jobs.filter(j => j.trigger === "recovery drill").length, 1);
    results.push("Scheduled recovery drill runs for the untested copy and does not repeat within the configured interval");
