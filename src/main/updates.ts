...
export class SoftwareUpdates {
  ...
  constructor(private enabled: boolean, private publish: () => void, private updater: Pick<AppUpdater, "autoDownload" | "autoInstallOnAppQuit" | "allowPrerelease" | "allowDowngrade" | "logger" | "on" | "checkForUpdates" | "downloadUpdate" | "quitAndInstall">) {
    ...
    this.updater.on("error", () => this.set({ status: "error", version: this.state.version, message: "Could not update Sentry. Check your connection and try again. The release may not be available yet." }));
  }
  set(state: UpdateState) { this.state = state; this.publish(); }
  async check() {
    if (!this.enabled || this.checking || ["available", "downloading", "ready", "installing"].includes(this.state.status)) return this.state;
    this.checking = true;
  ...
  async download() {
    if (!this.enabled || !this.state.version || !["available", "error"].includes(this.state.status)) return this.state;
    this.set({ status: "downloading", version: this.state.version, progress: 0 });
    try { await this.updater.downloadUpdate(); }
    catch { this.set({ status: "error", version: this.state.version, message: "Download failed. Check your connection and try again." }); }
    return this.state;
  }
