...
const host = await electron.launch({ executablePath, args: [resolve("."), "--service-host", "--background"], env });
let client: Awaited<ReturnType<typeof electron.launch>> | undefined;
try {
  assert.equal(await host.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().length), 0);
  ...
  await writeFile(join(root, "results.json"), JSON.stringify({ hostWindows: 0, backgroundConnected: true, desktopClosePreservedBackup: true, reconnectedToSameCatalog: true, recoveredFiles: result.files, scope: "Hidden Electron service-host lifecycle; not SCM installation, signed-out operation or reboot proof." }, null, 2));
